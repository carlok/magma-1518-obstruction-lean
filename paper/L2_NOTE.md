...
Research note, 4 September 2026. Status: **proved by a verified computation and a short written argument; not formally verified in Lean beyond its algebraic core; originality unresolved.** Companion to `research/BASELINE_AUDIT.md`, which records how the question was selected and what was searched.
...
(`research/audit/h2_certificate.json`, re-verified by multiplication in `R`, kernel-checked in `research/audit/H2Cert.lean`). Hence for every `R`-module `M` and every 1518-cocycle `f` (so `D_1518 f = 0`), `f = E(Pf)` is a coboundary: `H²_1518(G, M) = 0` for the Z/3 shift `G` and every finite 1518 fibre `M`.
...
```
sh research/audit/fetch_inputs.sh audit-data
sh research/audit/run_mace4.sh audit-data
python3 research/audit/l2_symbolic.py audit-data
python3 research/audit/l2_symbolic.py audit-data "one-element base"
python3 research/audit/monogenic_1518.py audit-data
lean +v4.33.1 -M 4096 research/audit/L2Cert.lean
```

Outputs as run on 2026-09-04 are in `research/audit/OUTPUT.md`. All scripts are standard-library Python, independent of `algebra_lab`, and do not touch any campaign's frozen code hashes.
